...
env = Environment.Environment(4, 1, 2 * 1060)
n_episode = 3000
...
logger_path = "2023-5-17-代码移植.log"
...
def get_state(env:Environment.Environment, idx=(0, 0), ind_episode=1., epsi=0.02):
    ...
    print(V2I_fast.shape,np.reshape(V2V_fast, -1).shape,V2V_interference.shape)
...
class Agent:
    def __init__(self, memory_entry_size):
        ...
        self.target_model = DQN(n_input_size, n_hidden_1, n_hidden_2, n_hidden_3, n_output_size).to(
            device)  # Target Model
    ...
    def predict(self, s_t, ep=0.):
        ...
        if random.random() > ep:
            ...
            return random.choice(range(n_power_levels * env.n_RB))
...
for i_episode in range(n_episode):
    ...
    # print('剩余数据',env.demand.reshape(4))
    p = np.count_nonzero(env.demand <= 0) / (env.n_Veh * env.n_neighbor)
    info = "episode - %d" % i_episode + "  " + "V2I平均速率：%fMbps" % (V2I_sum_capacity / n_step_per_episode) + "  " \
           + "V2V交付成功占比：%f" % p + "  " \
           + "平均奖励值：%fMbps" % np.mean(reward_list)
    logger.info(info)
